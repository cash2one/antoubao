...
if __name__ == '__main__':
    ...
    tabBasic['platform_qualitative_F'] = ["website", "financing_status", "registered_cap", "established_date", "location", "debt_transfer", "cash_in_fee", "cash_out_fee", "management_fee", "advanced_repayment", "reserve_fund", "third_entrust"]
    ...
    for date in dates:
        ...
        for v['platform_id'], v['platform_name'] in cur.fetchall():
            ...
            for (key, prop) in tabBasic.items():
                if (key == "platform_qualitative_F"):
                    ...
                    stringSQL = "SELECT "+','.join(prop)+" FROM "+key+" WHERE `platform_id` = '"+v['platform_id']+"' AND `date` = '"+date+"'"
            ...
            stringSQL = "INSERT INTO view_mobile("+strKey[:-1]+") VALUES("+strValue[:-1]+")"
        ...
        if lastdate != "0":
            ...
            for prop in tabBasic['platform_score_T']:
                if prop == "score" or prop == "capital_adequacy_ratio" or prop == "status":
                    continue
