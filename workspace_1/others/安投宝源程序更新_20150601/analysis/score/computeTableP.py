...
if __name__ == "__main__":
    ...
    for date_index in range(start, len(dates)):
        ...
        for _list in cur.fetchall():
            ...
            stringSQL = "SELECT avg("+E2DB+".weekly_ave_lending_per_borrower), avg("+E2DB+".weekly_ave_investment), avg("+E2DB+".weekly_ratio_new_old) FROM "+E2DB+", platform_score_H WHERE "+SRCDB+".platform_id = platform_score_H.platform_id AND platform_score_H.date = '"+kv['date']+"' ORDER BY platform_score_H.score DESC LIMIT 20"
            aCur.execute(stringSQL)
