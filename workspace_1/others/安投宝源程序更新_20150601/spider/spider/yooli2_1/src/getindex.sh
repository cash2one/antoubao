...
for((i=1;i<=2;i++))
do
	eval "${GETPAGE}" -O - "${URL}" "--header=\"Accept-Language:zh-CN,zh;q=0.8,en;q=0.6\"" "--post-data=\"${POSTDATA_INDEX}\"" \
		| awk '{s=$0;while(match(s,"/dingcunbao/detail/[0-9]+.html.*定存宝[^0-9]+[0-9]+.*$")) {printf("%s\n",substr(s,RSTART,RLENGTH));\
