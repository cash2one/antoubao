...
int parsefile(const char *idstr)
{
    ...
    kv["site_id"] = "myzhongjin";
    kv["borrower"] = extract(bstr, "业务情况", "：", "","</span>");
    kv["project_name"] = extract(bstr,"<div class=\"information\">","<h2>","","</h2>");
    kv["project_id"] = idstr;
...
//	kv["release_time"] = longtostring(stringtotime(extract(bstr,"起投时间","：","","</div>").c_str(),"%Y-%m-%d %H:%M"));
    ...
    kv["loan_period"] = loanperiod_util(extract(bstr, "期限", "：", "","</li>"));
