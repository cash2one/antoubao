...
int parsefile(const char *idstr)
{
    ...
    kv["loan_period"] = extract(bstr, "借款金额", "借款年利率", "借款期限：", "</div>");
    ...
    kv["invested_amount"] = doubletostring(s_atod(kv["borrowing_amount"].c_str()) - s_atod(filternum(extract(bstr,"笔投标","还需 ","\">￥","</span>")).c_str()));
    ...
    long rtime = s_atol(extract(bstr, "剩余时间", "id=\"endtime\">", "", "<").c_str());
    if (rtime > 0) {
