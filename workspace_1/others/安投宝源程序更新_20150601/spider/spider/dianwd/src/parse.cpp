...
int parsefile(const char *idstr)
{
    ...
    kv["site_id"] = "dianwd";
//    kv["borrower"] = extract(bstr, "", "姓名", "：","</div>");
    kv["project_name"] = extract(bstr,"<div class=\"hd\">","<h3>","","</h3>");
    kv["project_id"] = idstr;
    kv["borrowing_amount"] = num_util(extract(bstr, "项目金额", "￥","","</span>"));
	kv["invested_amount"] = doubletostring(s_atod(kv["borrowing_amount"].c_str()) - s_atod(num_util(extract(bstr, "可投金额", "￥", ">", "</span>")).c_str()));
...
//	kv["release_time"] = longtostring(stringtotime(extract(bstr,"开始时间","\">","","</span>").c_str(),"%Y-%m-%d %H:%M:%S"));
    ...
    kv["loan_period"] = loanperiod_util(extract(bstr, "还款期限", "<div", ">","</div>"));

    {
        ...
        while (p < buf + buflen - 8) {
            ...
            while ((ivname = extract(tmp, "<td>", "", "", "</td>")) != "") {
                ...
                ivtime = extract(tmp, "<td class=\"time\">", "", "", "</td>");
