...
int parsefile(const char *idstr)
{
    ...
    kv["site_id"] = "15dai";
    kv["borrower"] = extract(bstr, "用户名", ": ", "","</p>") + "_" + extract(bstr, "站内关联用户名", "：", "","</p>");
    kv["project_name"] = extract(bstr,"<div class=\"box-info-detail\">","<h2>","\">","</span>");
    kv["project_id"] = idstr;
    kv["borrowing_amount"] = num_util(extract(bstr, "借款金额", "￥","<strong>","</strong>"));

	kv["release_time"] = longtostring(stringtotime(extract(bstr,"发标时间","：","","</p>").c_str(),"%Y-%m-%d %H:%M:%S"));
