...
int parsefile(const char *idstr)
{
    ...
    kv["site_id"] = "qdp2p";
    kv["project_name"] = extract(bstr, "<div class=", "<div class=\"pt_box\">", "\">","</div>");
    kv["borrower"] = extract(bstr,"<img","<div class=\"utx-r\">","<strong>","</strong>");
    kv["project_id"] = extract(bstr,"<form id=","<input type=\"hidden\"","value=\"","\">");
    kv["borrowing_amount"] = extract(bstr,"借款金额","<div class=\"pt_lilr\">","font\">","</span>");
    ...
    kv["annulized_rating"] = filternum(extract(bstr,"年利率","<div class=\"pt_lilr\">", "\">", "</span>%"));
    kv["payment_method"] = extract(bstr,"还款方式","<div class=\"pt_lilr\">","<span>","</span>");
    kv["loan_period"] = extract(bstr, "还款期限", "<div class=\"pt_lilr\">", "\">","</div>");
 ...
 //    kv["invested_amount"] = doubletostring(s_atod(kv["borrowing_amount"].c_str()) - s_atod(filternum(extract(bstr,"剩余时间","额","￥","")).c_str()));
  //   kv["release_time"] = extract(bstr,"还款方式","发布时间","\">","");
   //     kv["end_time"] = extract(bstr,"<span class=\"f_16\"","<input id=\"hidden\"","value=\"","\" />");
    kv["project_details"] = extract(bstr,"<span class=\"c2boxcl1\">","src=","<div class=\"c2boxc c2pt15\">","</div>");
    ...
    if (buf) {
        ...
        while (p < buf + buflen - 8) {
            ...
            if (!get_string_member(json,"list", jstr)) {
                continue;
            }
            Value jlist = stringtojson(jstr);
            if (!jlist.isArray()) {
                continue;
            }
            for (int i = 0; i < jlist.size(); i++) {
                ...
                string addtime;
                string username;
                if (!get_double_member(jlist[i], "tenderAmount", account)
                        || !get_string_member(jlist[i], "tendetDate", addtime)
                        || !get_string_member(jlist[i], "username", username)) {
                    continue;
                }
