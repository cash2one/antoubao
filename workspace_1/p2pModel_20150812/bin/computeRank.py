...
def rank(scored, scoreRange):
    _score = {}
    scoreDict = {np.percentile(scoreRange, 98): "A++", \
                np.percentile(scoreRange, 81.3): "A+", \
                np.percentile(scoreRange, 55.4): "A", \
                np.percentile(scoreRange, 51):"B++", \
                np.percentile(scoreRange, 3.3113+16.8874):"B+", \
                np.percentile(scoreRange, 5.9):"B"}
    ...
    for timestamp in arr.keys():
        ...
        for key in arr[timestamp].keys():
            ...
            scoreArr[timestamp][key] = [\
                np.percentile(tmp, 98), \
                np.percentile(tmp, 81.3), \
                np.percentile(tmp, 52), \
                np.percentile(tmp, 51), \
                np.percentile(tmp, 3.3113+16.8874), \
                np.percentile(tmp, 5.9)]
