...
#准备数据
quanti, qualit, quasco = readQuanti(rQUANTI), readQualit(rQUALIT), readQuasco(rQUASCO)
...
for key in planBKeys:
    ...
    if ave != 0:
        ratio[key] = np.average(datas[date][key], weights=datas[date]['weekly_lending'])/ave
...
DEVCUR.execute("TRUNCATE view_mobile")
DEVCONN.commit()
...
#自动评论
count = 0
stringSQL = "SELECT platform_id, level, date, old_date FROM view_mobile"
DEVCUR.execute(stringSQL)
for platid, level, date, old_date in DEVCUR.fetchall():
