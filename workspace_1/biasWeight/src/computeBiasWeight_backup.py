...
if __name__ == '__main__':
    ...
    index_list = getAllColumnsFromTable(cur_dev, SRCDB_E3, del_list)
    ...
    iteration_weight_list = [x * 0.0025 for x in range(1, 400 * 2 / weight_number + 1, 1)]
    iteration_weight_number = len(iteration_weight_list)
    print "每个指标的迭代空间为: "
    print iteration_weight_list
    print 
    # 确定权重系数
    # 每四个一组按照出现指数Q来确定贝叶斯权重
    ...
    if block_reminder != 0:
        block_number += 1
    fp = open("weight_0_" + str(block_number) + ".txt", 'w')
    for weight_final in weight_value_order_list1:
        fp.write("%.10f" % weight_final + " \n")
    fp.close()
    fp = open("block_number.txt", 'w') #分组数
    fp.write(str(block_number) + " \n")
    fp.close()
    #分组循环
    for i in range(block_number): #10组
        if i == block_number - 1:
            weight_index = range(BLOCKCONTAINS * i, weight_number)
        else:
            weight_index = range(BLOCKCONTAINS * i,BLOCKCONTAINS * (i + 1))
        weight_index_number = len(weight_index) #每组4个
        if weight_index_number == 0:
            continue
        print "正在进行第" + str(i+1) + "/" + str(block_number) + "组计算。"
        sys.stdout.flush()
        #组内循环
        iteration_number = iteration_weight_number ** weight_index_number #50 ** 4 
        Q_sum = 0
        new_weight_index = [0] * weight_index_number
        counter = 0
        PercentList = [floor(float(x) * iteration_number / BUFFERNUMBER) for x in range(1, BUFFERNUMBER + 1)] 
        for j in range(iteration_number):
            counter += 1
            if counter in PercentList:
                print "count: " + str((1 + PercentList.index(counter)) * 100 / BUFFERNUMBER) + "%"
            ...
            for platform in platform_dict:
                score[platform] = getVectorInnerProduct(value_dict[platform], weight_value_order_list1)
            ...
            Q_sum += Q_index
            #print grade_hold_number
            for k in range(weight_index_number):
                new_weight_index[k] += weight_value_order_list1[weight_index[k]] * Q_index
        for k in range(weight_index_number):
            if Q_sum == 0:
    ...
    file_name = "weight_" + str(RANKFLOATING) + ".txt"
    fp = open(file_name, 'w')
    for weight_final in weight_value_order_list1:
        fp.write("%.10f" % weight_final + " \n")
    fp.close()
...
#     picture_name = "weight_" + str(RANKFLOATING) + ".jpg"
