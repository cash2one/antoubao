...
if __name__ == "__main__":
    ...
    #same_1_start: the same to computeTableE2_ABP.py    
    for date_index in range(start, len(dates)):
        ...
        for _list in scur.fetchall():
            ...
            cur.execute(stringSQL)
            conn.commit()
            #same_1_end: the same to computeTableE2_ABP.py
        ...
        for platId in proArr:
            stringSQL = "INSERT INTO "+DSTDB+"(`date`, `"+"`, `".join(arrKeys)+"`) VALUE('"+str(date)+"', '"+platId+"B', 'strategy_report', '"+"', '".join(['0']*(len(arrKeys)-2))+"')"
            cur.execute(stringSQL)
            conn.commit()
            result = {}
            ...
            for prop in strategyB:
                for prop_map in strategyB[prop]:
                    stringSQL = "SELECT "+prop_map+" FROM "+DSTDB+" WHERE `date` = '"+str(date)+"' AND `platform_id` = '"+platId+"'"
                    ...
                    stringSQL = "SELECT platform_id FROM "+DSTDB+" WHERE `"+prop_map+"` > '"+str(range2)+"' AND `"+prop_map+"` < '"+str(range1)+"' AND `date` = '"+str(date)+"'"
                    ret = cur.execute(stringSQL)
                    if ret == 0:
                        result[prop] = 0
                    else:
                    ...
                    stringSQL = "UPDATE "+DSTDB+" SET `"+prop+"` = '"+str(result[prop])+"' WHERE `platform_id` = '"+platId+"B' AND `date` = '"+str(date)+"'"
                    cur.execute(stringSQL)
                    conn.commit()
