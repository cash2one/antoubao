...
if __name__ == "__main__":
    ...
    for date_index in range(start, len(dates)):
        ...
        for fun in ["max", "min", "variance", "avg"]:
            ...
            valueStr = ""
            if fun == "avg":
                _where += " AND `registered_cap` < '50000' AND `weekly_ave_lending_per_borrower` > `weekly_ave_lending_per_bid`*0.9"
            for key in arrKeys:
                if date <= LINEDATE:
                    stringSQL = "SELECT "+fun+"("+key+") FROM "+SRCDB+" WHERE `date` = '"+str(date)+"' AND `"+key+"` > '0' AND "+_where
                else:
                    stringSQL = "SELECT "+fun+"("+key+") FROM "+SRCDB+" WHERE `date` = '"+str(date)+"' AND `"+key+"` > '0' AND `"+key+"` < '"+str(ceil[key])+"' AND "+_where
                cur.execute(stringSQL)
        ...
        if ret == 0:
            stringSQL = "DELETE FROM "+table["avg"]+" WHERE `date` = '"+str(date)+"'"
            inlineCur.execute(stringSQL)
            conn.commit()
            continue
