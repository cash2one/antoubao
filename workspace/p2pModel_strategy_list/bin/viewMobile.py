...
for key in planBKeys:
    ...
    if ave != 0:
        ratio[key] = np.average(datas[date][key], weights=datas[date]['weekly_lending'])/ave #整个行业中每个planB字段值得最终时间和起始时间的比值（用weekly_lending加权）

for key in planCKeys:
    ratio[key] = np.mean(datas[date][key])/np.mean(datas[str(STARTDATE)][key]) #整个行业中每个planC表字段值得最终时间和起始时间的比值（直接求平均值）

#读取自动评论配置
scoreArr = {} #保存了view_score_condition按照前面计算出的全行业的最终时间和起始时间的比例，推算出planB和planC指标最终时间时应该对应的值，根据这个值来决定评论时所对应的等级
...
DEVCUR.execute(stringSQL)
for col in DEVCUR.fetchall():
    summaryArr[col[0]] = [col[1], col[2], col[3], col[4], col[5]] #各个指标评论的形容词
    contentArr[col[0]] = col[6] #指标名称

DEVCUR.execute("DELETE FROM view_mobile")
DEVCONN.commit()

#将Redis-db(9)中最新周的数据写到ddpt-data上的view_mobile表
del_field_list = ["score_initial", "level_initial", "punishment_info", "punishment", "hehepunishment", "score_after_punishment", "level_after_punishment", "smooth_info", "score_after_smooth", "level_after_smooth", "downgrading_info", "score_after_degrade", "level_after_degrade", "degrade_smooth_info", "level_lock", "none_downgrading_info"]
for platid in rRESULT.keys():
    if date in rRESULT.hkeys(platid):
        ...
        for field in del_field_list:
            if field in d:
                del d[field]
        fromHashToMySQL(DEVCUR, DEVCONN, "view_mobile", d)
