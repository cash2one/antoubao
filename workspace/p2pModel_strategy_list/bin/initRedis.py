...
if len(sys.argv) < 2:
    ...
    rQUANTI.flushdb()
    #读取定量表E1所有内容。
    d = fromMySQLToArray(DATACUR, "0_E1_quantitative_data", " WHERE `date` >= "+str(STARTDATE)+" ORDER BY `date` ASC")
    if len(d) == 0:
        print "Failure in reading in platform_quantitative_data_E1."
        exit(-1)
    s = fromMySQLToArray(DATACUR, "2_total_status")
    ...
    if len(sys.argv) < 3:
        ...
        if timestamp < STARTDATE:
            ...
            exit(-1)
    d = fromMySQLToArray(DATACUR, "0_E1_quantitative_data", " WHERE `date` = '"+str(timestamp)+"'")
    if len(d) == 0:
        print "Failure in reading in 0_E1_quantitative_data."
        exit(-1)
    s = fromMySQLToArray(DATACUR, "2_total_status")
    count = readQuantitative(d, s)
    print "Quantitative data("+str(count)+") written to redis-db("+str(dbQUANTI)+")!"
    ...
    rQUALIT.flushdb()
    #读取定性表F
    d = fromMySQLToArray(DATACUR, "1_platform_qualitative_F")
    if len(d) == 0:
        print "Failure in reading in 1_platform_qualitative_F."
        exit(-1)
    ...
    rSTATUS.flushdb()
    #读取平台状态表Y
    d = fromMySQLToArray(DATACUR, "2_total_status")
    if len(d) == 0:
        print "Failure in reading in 2_total_status."
        exit(-1)
    #遍历状态表内容，依次写入Redis的db(3)
    count = 0
    for jsonStr in d:
        pinfo = json.loads(json.dumps(jsonStr))
        ret = fromHashToRedis(rSTATUS, pinfo, 'platform_id', 'date', 'platform_name|platform_id|status|summary|date|description|address|level')
        if ret == 0:
            print "Failure in writing out "+pinfo['platform_name']+"."
        else:
            count+=1
