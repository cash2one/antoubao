...
#写入xml文件
def writeGradeXmlFilx(_dom):
    xml_final_grade_file = "atbmodel_grade_final.xml"
    fp = open(xml_final_grade_file, "w+")
    _dom.writexml(fp, "", "\t", "", "UTF-8")
    fp.seek(0)
...
#写入xml文件
def writeIndexXmlFilx(_weight_dict):
    field_list_order = sortDictByValue(_weight_dict)[0]
    xml_final_indexes_file = "atbmodel_index_final.xml"
    ...
    for field in field_list_order:
        ...
        field_node.setAttribute("weight", str(_weight_dict[field]))
...
def checkXmlWeightFromXmlFile(_file):
    ...
    writeGradeXmlFilx(dom)
    writeIndexXmlFilx(weight_dict)
    return True
...
def resetXmlWeightFromXmlFile(_file, _fix_field):
    ...
    if not checkXmlWeightFromXmlFile("atbmodel_grade_final.xml"):
        return False
    else:
        return True
...
if __name__ == '__main__':
    ...
    if operation == "-c":    
        if not checkXmlWeightFromXmlFile('atbmodel_grade_final.xml'):
            print "请重新设置指标权重文件'atbmodel_grade_final.xml'!"
            exit(1)
        else:
            print "指标权重正常."
