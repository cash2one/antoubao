...
def writeStatistics(pinfos, QUALIT_TITLE):
    ...
    for pinfo in pinfos:
        ...
        s = 0
        i = 0
        if (d['date']-STARTDATE)/(7*24*3600) >= 3 :
            for t in range(0, (d['date']-STARTDATE)/(7*24*3600) + 1):
...
if len(sys.argv) < 2:
    ...
    for platId in rQUANTI.keys():
        for field in rQUANTI.hkeys(platId):
            if field not in pinfoDict:
                pinfoDict[field] = []
            pinfoDict[field].append(rQUANTI.hget(platId, field))
    #遍历pinfoDict，将每个日期的平台的数据传给writeStatistics
    for date in pinfoDict.keys():
        if date < SCOREDATE:
            continue
        # pinfoDict[date]：每一个时间点不同的站的字段值字典组成的列表
