...
#获取punishment策略
def getPunishmentDict():
    dom = xml.dom.minidom.parse('atbmodel_punish_final.xml')
    punish_root = dom.getElementsByTagName("punish")[0]
    punish_dict = {}
    for punish_node in findChildrenNodes(punish_root):
        punish_node_name = punish_node.nodeName
        deduction = float(punish_node.getAttribute("deduction"))
        punish_dict["#" + punish_node_name + "#"] = deduction
    return punish_dict

#获取warning策略
def getWarningDict():
    dom = xml.dom.minidom.parse('atbmodel_warning_final.xml')
    warning_root = dom.getElementsByTagName("warning")[0]
    warning_dict = {}
    for warning_node in findChildrenNodes(warning_root):
        warning_node_name = warning_node.nodeName
        warning_dict[warning_node_name] = {}
        if "w5" in warning_node_name:
            warning_dict[warning_node_name]["weight_min"] = float(warning_node.getAttribute("weight_min"))
            warning_dict[warning_node_name]["weight_max"] = float(warning_node.getAttribute("weight_max"))
        else:
            warning_dict[warning_node_name]["weight"] = float(warning_node.getAttribute("weight"))
        warning_dict[warning_node_name]["score"] = float(warning_node.getAttribute("score"))
    return warning_dict

#获取warning策略
def getDegradeDict():
    dom = xml.dom.minidom.parse('atbmodel_degrade_final.xml')
    degrade_root = dom.getElementsByTagName("degrade")[0]
    degrade_dict = {}
    for level_node in findChildrenNodes(degrade_root):
        level_node_name = level_node.nodeName.replace("P", "+")
        degrade_dict[level_node_name] = {}
        for degrade_node in findChildrenNodes(level_node):
            degrade_node_name = degrade_node.nodeName
            degrade_dict[level_node_name][degrade_node_name] = {}
            for attribute in ["info", "threshold", "symbol"]:
                degrade_dict[level_node_name][degrade_node_name][attribute] = degrade_node.getAttribute(attribute)
    return degrade_dict
...
def simplePunish(quanti, qualit, scored, top20ave, w, _punish_dict):
    if 'punishment' not in scored.keys():
        scored['punishment'] = 0
    if 'punishment_info' not in scored.keys():
        scored['punishment_info'] = ""
    #定量数据惩罚 @闫欣，杨泽斌
#     if quanti['weekly_lending'] < 350:
#         punish_method = '#PU001#'
#         scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
#     if quanti['weekly_total_investor'] < 300:
#         punish_method = '#PU002#'
    if quanti['weekly_lending'] < 350 and quanti['weekly_total_investor'] < 300:
        punish_method = '#PU001#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['weekly_new_investor'] < 100:
        punish_method = '#PU003#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['weekly_ratio_new_old'] < 0.01:
        punish_method = '#PU004#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['ave_annualized_return'] > 17.5:
        punish_method = '#PU0051#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['ave_annualized_return'] > 21:
        punish_method = '#PU0052#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['weekly_ave_bid_close_time'] > 5000:
        punish_method = '#PU0061#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['weekly_ave_bid_close_time'] > 49001:
        punish_method = '#PU0062#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['weekly_loan_period'] < 75:
        punish_method = '#PU007#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['weekly_total_borrower'] < 15:
        punish_method = '#PU011#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['top10_ratio_loan'] > 0.6:
        punish_method = '#PU0121#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['top10_ratio_loan'] > 0.9:
        punish_method = '#PU0122#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['borrower_growth'] < 0.95:
        punish_method = '#PU021#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['investor_growth'] < 0.95:
        punish_method = '#PU022#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['money_growth'] < 0.8:
        punish_method = '#PU023#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['outstanding_loan'] > 0.7:
        punish_method = '#PU031#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
#     if quanti['short_term_debt_ratio'] > 0.4:
#         punish_method = '#PU032#'
    if (quanti['short_term_debt_ratio'] > 0.4 and quanti['not_returned_yet'] < 1.1) or (quanti['short_term_debt_ratio'] > 0.5 and quanti['not_returned_yet'] < 1.3):
        punish_method = '#PU032#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['not_returned_yet'] < 0.7:
        punish_method = '#PU0331#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['investor_HHI'] > 2100:
        punish_method = '#PU041#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['borrower_HHI'] > 5100:
        punish_method = '#PU042#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['weekly_ave_lending_per_bid'] != 0:    
        if quanti['weekly_ave_lending_per_borrower'] / quanti['weekly_ave_lending_per_bid'] > 4:
            punish_method = '#PU043#'
            scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['weekly_ratio_new_old'] > 0.4:
        punish_method = '#PU044#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['borrower_growth'] > 1.22:
        punish_method = '#PU045#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['investor_growth'] > 1.3:
        punish_method = '#PU046#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['money_growth'] > 1.25:
        punish_method = '#PU047#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
    if quanti['top10_ratio_investment'] > 0.5:
        punish_method = '#PU051#'
        scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
...
#         punish_method = '#PU052#'
#         scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
#     if 0.49 < qualit['third_entrust'] < 0.51:
#         punish_method = '#PU061#'
#         scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
#     elif qualit['third_entrust'] < 0.1:
#         punish_method = '#PU062#'
#         scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
#     if qualit['real_name'] < 0.7:
#         punish_method = '#PU07#'
#         scored['distribution'] -= 2/_punish_dict[punish_method]
#         scored['mobility'] -= 2/_punish_dict[punish_method]
#         scored['security'] -= 2/_punish_dict[punish_method]
#         scored['punishment'] += 2
...
#         punish_method = '#PU082#'
#         scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
#     if quanti['weekly_ratio_new_old'] > 5.0/6:
#         punish_method = '#PU091#'
#         scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
#     elif quanti['weekly_ratio_new_old'] > 0.5:
#         punish_method = '#PU092#'
#         scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
#     if quanti['weekly_ratio_new_old'] > top20ave['weekly_ratio_new_old']*8.4:
#         punish_method = '#PU093#'
#         scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
#     elif quanti['weekly_ratio_new_old'] > top20ave['weekly_ratio_new_old']*2.6:
#         punish_method = '#PU094#'
#         scored = punishment(scored, _punish_dict[punish_method], w, punish_method)
...
#纵向惩罚
def linePunish(scored, quanti, platid, timestamp, w, INVALID_TITLE, _punish_dict):
    quantiList = {}
...
def fourWeekAve(scored, timestamp, INVALID_TITLE):
    ...
    for ratio in ratios:
        for key in scored[timestamps[pos+i]].keys():
            ...
            if key == "score":
                smooth_info += "%.4f" % scored[timestamps[pos+i]]["score"] + "(" + scored[timestamps[pos+i]]["level"] + ") * " + str(ratio) + " + "
        i += 1
    result[timestamp]["smooth_info"] = smooth_info[:-3]
...
def computeQuantitativeScore(pinfo, qualit, sinfo):
    ...
    v['cash_flow_in'] = (exp(pinfo['cash_flow_in'])-exp(statis['3SIG']['cash_flow_in_l']))/(exp(statis['3SIG']['cash_flow_in_h'])-exp(statis['3SIG']['cash_flow_in_l']))*100
    if v['cash_flow_in'] > 100:
        v['cash_flow_in'] = 100
    if v['cash_flow_in'] < 0:
        v['cash_flow_in'] = 0
    #foo2
...
if __name__ == '__main__':
    print getPunishmentDict()
    print getWarningDict()
    print getDegradeDict()