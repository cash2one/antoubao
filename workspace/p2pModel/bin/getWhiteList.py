...
if __name__ == '__main__':
    ...
    initializeCursors(cur_dev, cur_db, cur_ddpt)
    
    SRCDB_Y = "total_status" #db  
    SRCDB_E2 = "E2_quantitative_data" #dev
    SRCDB_Z = "platform_problem_doubt_Z" #dev
    SRCDB_V = "V_view" #ddpt
    ...
    status_dict_good = {}
    date_dict_good = {}
    stringSQL = "SELECT A.platform_name, A.`date`, A.status FROM total_status AS A,(SELECT `platform_name`, MAX(`date`) AS `date` FROM total_status GROUP BY `platform_name`) AS B WHERE A.platform_name = B.platform_name AND A.`date` = B.`date`"
    ...
    for platform_name in status_dict_good:
        value = getValue(cur_dev, SRCDB_E2, ["weekly_lending", "weekly_total_investor"], platform_name, this_date, 6, 0.6, 0.4)
        if value > 0 :
            score_dict[platform_name] = value
            lack_dict[platform_name] = checkLack(cur_dev, SRCDB_E2, ["weekly_lending", "weekly_total_investor"], platform_name, this_date)
    good_platform_list_sorted = sortDictByValue(score_dict)[0]
    good_platform_number = len(good_platform_list_sorted)

    platform_list_last = getDifferentFieldlistByDate(SRCDB_Z, cur_dev, "platform_name", last_date)
    ...
    ATB_top20_dict = {}
    date_list_V = getDifferentFieldlist(SRCDB_V, cur_ddpt, "date")
    ...
    stringSQL = "SELECT `platform_name`, `rank_score` FROM " + SRCDB_V + " WHERE `date` = '" + str(max_date) + "' AND `rank_score` <= '20'" 
    ...
    for platform_name in status_dict_good:
        ...
        cur_db.execute(stringSQL)
        conn_db.commit()
    closeCursors(cur_dev, cur_db, cur_ddpt)
    closeConns(conn_dev, conn_db, conn_ddpt)
    _end_time = time.time()
