...
if __name__ == '__main__':
    ...
    platform_id_list = getPlatformIdList("platform_id.txt")
    ...
    cur_db.execute(stringSQL)
    _end_time = time.time()
    print "The sql command  costs " + str(_end_time - _start_time) + " seconds."  
    _start_time = time.time()
    ...
    _end_time = time.time()
    print "The pre_treat program costs " + str(_end_time - _start_time) + " seconds."  
    
    # 插入Table01
    _start_time = time.time()
    print "Reset '" + TABLE_01
#     cur_db.execute("TRUNCATE " + TABLE_01)
#     conn_db.commit()
    ...
    for platform_id in platforms_investors:
        for investor in platforms_investors[platform_id]:
            ...
            conn_db.commit()
    _end_time = time.time()
    print "The Table 01 reset program costs " + str(_end_time - _start_time) + " seconds."  
    
    # 插入Table02
    _start_time = time.time()
    print "Reset '" + TABLE_02
#     cur_db.execute("TRUNCATE " + TABLE_02)
#     conn_db.commit()
    PercentList = [floor(float(x) * investorNumber / BUFFERNUMBER) for x in range(1, BUFFERNUMBER + 1)]
    counter = 0
    for platform_id in platforms_investors:
    ...
    _end_time = time.time()
    print "The Table 02 reset program costs " + str(_end_time - _start_time) + " seconds."  
    
    # 插入Table03
    _start_time = time.time()
    print "Reset '" + TABLE_03
#     cur_db.execute("TRUNCATE " + TABLE_03)
#     conn_db.commit()
    ...
    for platform_id in platforms_borrowers:
        for borrower in platforms_borrowers[platform_id]:
            ...
            conn_db.commit()
    _end_time = time.time()
    print "The Table 03 reset program costs " + str(_end_time - _start_time) + " seconds."  
    
    # 插入Table04
    print "Reset '" + TABLE_04
#     cur_db.execute("TRUNCATE " + TABLE_04)
#     conn_db.commit()
    PercentList = [floor(float(x) * borrowerNumber / BUFFERNUMBER) for x in range(1, BUFFERNUMBER + 1)]
    counter = 0
    for platform_id in platforms_borrowers:
        for borrower in platforms_borrowers[platform_id]:
            ...
            for i in range(dates_number):
                ...
                if week_amount_temp > money_error:
                    ...
                    conn_db.commit()
    _end_time = time.time()
    print "The Table 04 reset program costs " + str(_end_time - _start_time) + " seconds."  
    
    # 插入Table05
    _start_time = time.time()
    print "Reset '" + TABLE_05 + "'"
#     cur_db.execute("TRUNCATE " + TABLE_05)
#     conn_db.commit()
    # 所有平台参与者的总人数：
    ...
    for platform_id in platforms_investors:  # 与for platform_id in platforms_borrowers一致:
        ...
        for i in range(0, DURATIONWEEKS):  
            if have_date[i] == 1:          
                ...
                conn_db.commit()   
    _end_time = time.time()
    print "The Table 05 reset program costs " + str(_end_time - _start_time) + " seconds."  
    
    # 插入Table06
    _start_time = time.time()
    print "Reset '" + TABLE_06 + "'"
#     cur_db.execute("TRUNCATE " + TABLE_06)
#     conn_db.commit()
    counter = 0
    ...
    for platform_id in platforms_investors:
        ...
        for i in range(start_date_index, dates_number):  # 为避免本周数据的干扰，特别的将本周数据去掉
            ...
            if having_date[i] == 1: 
                ...
                conn_db.commit()
    _end_time = time.time()
    print "The Table06 reset program costs " + str(_end_time - _start_time) + " seconds."  
    print "The whole reset program costs " + str(_end_time - _start_time_whole) + " seconds."  
    closeCursors(cur_db)
    closeConns(conn_db)  
