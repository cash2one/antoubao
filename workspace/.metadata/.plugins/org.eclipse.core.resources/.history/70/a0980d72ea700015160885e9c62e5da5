...
if __name__ == '__main__':
    ...
    for line in fp:
        ...
        if "BLACKLIST" in line:
            exec(line)
        if "LEVEL_PERCENTAGE_DICT" in line:
            exec(line)
    fp.close()
    LEVEL_PERCENTAGE_DICT["C"] = 0
    ...
    for level in LEVEL_LIST:
        total_dict[level + "站的百分位和个数"]["child"].append("(" + str(LEVEL_PERCENTAGE_DICT[level]) + "): " + str(level_number_dict[level]))
        _string = level + "(" + str(LEVEL_PERCENTAGE_DICT[level]) + "): " + str(level_number_dict[level])
        print _string 
        info_file.write(_string + "\n")
    print 
    info_file.write("\n")
