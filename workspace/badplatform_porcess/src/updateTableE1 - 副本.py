...
if __name__ == '__main__':
    ...
    for ID, _date, platform_id, borrower, borrowing_amount, loan_period, annulized_rating, payment_method, investorStr, release_time, end_time in rows:
        ...
        for i in range(0, len(investorArr) - 1):
            if i % 3 == 1 :
                ...
                money_str = investorArr[i + 1].replace(",", "")
                if money_str.endswith("元"):
                    money_str = money_str[0:money_str.index("元")]
                try: 
                    total_gross = float(money_str) / MONEYUNIT #万元 # 防止出现2,000.00的情况
                except:
                    total_gross = 0
    ...
    print "开始更新'" + dstdb_E1 + "'"
    #same_1_start: the same to computeTable010203040506.send_mail
...
#             registered_cap[i] += 0.1 * parent_company_cap[i] #万元
            ...
            for i in range(0, borrowerbidNumber):
                ...
                for j in range(0, dates_number):
                    ...
                    if interest_end_date > history_time_temp >= invest_time:
                        ...
                        futureInterest = total_gross * (wholeInterest - thisInterest)
                        weekly_outstanding_loan[j] += futureInterest #万元
                        pending_repayment[j] +=futureInterest
        ...
        for bidID in platforms_bids[platform_id]:
            ...
            weekly_loan_period_initial[history_time_node] += float(bidTemp["loan_period"]) * bidTemp["borrowing_amount"] #天*万元
            ave_annualized_return_initial[history_time_node] += bidTemp["annulized_rating"] * bidTemp["borrowing_amount"] #0.01*万元
...
#                         break
        ...
        #same_1_end: the same to computeTable010203040506.send_mail
...
#         
    ...
    #resetMarketShareGrowth(cur_db, conn_db, dstdb_E1)
