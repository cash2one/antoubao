...
if __name__ == '__main__':
    ...
    for platform_name, percentage_total in cur_db.fetchall():
        if percentage_total != None and percentage_total <= 100 * (1 - threshold2):
            platform_name_not_error_list.append(platform_name)
    print "对于总体错误率来说，共有" + str(len(platform_name_not_error_list)) + "个平台的错误率小于" + str((1 - threshold2)) + "."
    
    add_platform_name_list = list(set(add_platform_name_list) & set(platform_name_not_error_list))
    print "在每周正确率大于" + str(threshold1) +"或三周平均正确率大于" + str(threshold2) + "的情况下，共有" + str(len(platform_name_not_error_list)) + "个平台满足要求."

    add_platform_name_list = list(set(add_platform_name_list) - set(platform_name_E1_list))
    print "其中可以向E1多补充" + str(len(add_platform_name_list)) + "个平台."
    ...
    for rets in cur_db.fetchall():
        ...
        for value in value_list:
            ...
            platform_name_have_data_list.append(platform_name)
    del_platform_name_list = list(set(add_platform_name_list) - set(platform_name_have_data_list))
    print "其中有" + str(len(del_platform_name_list)) + "个平台在最近" + str(last_weeks) + "周并没有抓取到数据"  
    print "它们是："
    for platform_name in del_platform_name_list:
        print platform_name,
    print
    
    add_platform_name_list = list(set(add_platform_name_list) & set(platform_name_have_data_list))
    print "则只能向E1补充" + str(len(add_platform_name_list)) + "个平台."
    print "它们是："
